...
Checks:
- YAML front matter can be parsed
...
- updated >= created
- relationship fields are lists and reference existing objects
- deprecated/reserved ResearchOS metadata names
- template placeholders accidentally left in real objects
...
try:
    ...
    print(
        ...
        "Install it in the researchos environment with:\n\n"
        "    python -m pip install pyyaml\n",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
# These names are intentionally avoided in ResearchOS object metadata because
# they collide with Quarto semantics or with the plural relationship schema.
FORBIDDEN_KEYS = {
...
def read_record(path: Path) -> Record:
    ...
    yaml_text = split_front_matter(text)
...
def discover_object_paths() -> list[Path]:
    ...
    # Numbered objects: <collection>/<id>-<slug>/index.qmd
    ...
    # Research logs: logs/YYYY/YYYY-MM-DD.qmd
    logs = ROOT / "logs"
    if logs.exists():
    ...
    for key, suggestion in FORBIDDEN_KEYS.items():
        if key in d:
            add(
                ...
                "ERROR",
                path,
                f"forbidden/deprecated field '{key}'; {suggestion}",
            )
    ...
    for token in PLACEHOLDERS:
        if token in record.text:
            add(
                ...
                "ERROR",
                path,
                f"template placeholder still present: {token!r}",
            )
    ...
    for field in RELATION_FIELDS:
        ...
        if value is None:
            add(
                ...
                "WARNING",
                path,
                f"relationship field '{field}' is null; prefer []",
            )
            ...
            add(
                ...
                "ERROR",
                path,
                f"relationship field '{field}' must be a YAML list",
            )
        else:
            for item in value:
                if not isinstance(item, str):
                    add(
                        ...
                        "ERROR",
                        path,
                        f"'{field}' entries must be strings; found {item!r}",
                    )
...
def validate_numbered(record: Record, findings: list[Finding]) -> None:
    ...
    if object_type not in OBJECT_RULES:
        add(
            ...
            "ERROR",
            path,
            f"unknown numbered object_type: {object_type!r}",
        )
        return
    ...
    if path.parent.parent.resolve() != expected_folder.resolve():
        add(
            ...
            "ERROR",
            path,
            f"object_type '{object_type}' must live under "
            f"{rule['folder']}/",
        )
    ...
    if not isinstance(object_id, str) or not id_pattern.fullmatch(object_id):
        add(
            ...
            "ERROR",
            path,
            f"'id' must match {rule['prefix']}-NNN; found {object_id!r}",
        )
    else:
        directory_name = path.parent.name
        expected_dir_pattern = re.compile(
            rf"^{re.escape(object_id)}-[a-z0-9]+(?:-[a-z0-9]+)*$"
        )
        if not expected_dir_pattern.fullmatch(directory_name):
            add(
                findings,
                "ERROR",
                path,
                f"directory '{directory_name}' must begin with "
                f"'{object_id}-' and use a lowercase ASCII slug",
            )
    ...
    if status is not None and status not in rule["statuses"]:
        ...
        add(
            ...
            "ERROR",
            path,
            f"invalid status {status!r} for {object_type}; allowed: {allowed}",
        )
    ...
    if d.get("created_at") is not None and created_at is None:
        add(
            ...
            "ERROR",
            path,
            "'created_at' must be an offset-aware ISO 8601 timestamp",
        )
    if d.get("updated_at") is not None and updated_at is None:
        add(
            ...
            "ERROR",
            path,
            "'updated_at' must be an offset-aware ISO 8601 timestamp",
        )
    ...
    # Literature-specific ResearchOS schema.
    if object_type == "literature":
        ...
        if "publication" not in d:
            add(
                ...
                "WARNING",
                path,
                "literature record has no 'publication' field",
            )
...
def validate_log(record: Record, findings: list[Finding]) -> None:
    ...
    if d.get("object_type") != "research-log":
        add(
            ...
            "ERROR",
            path,
            "log object_type must be 'research-log'",
        )
    ...
    if log_date:
        ...
        expected_id = f"log-{date_string}"
        expected_filename = f"{date_string}.qmd"
        expected_year = str(log_date.year)
        ...
        if path.name != expected_filename:
            add(
                ...
                "ERROR",
                path,
                f"log filename must be '{expected_filename}'",
            )
        if path.parent.name != expected_year:
            add(
                findings,
                "ERROR",
                path,
                f"log must be stored under logs/{expected_year}/",
            )
...
def validate_relations(records: list[Record], findings: list[Finding]) -> None:
    existing_ids = {
        r.data.get("id")
        for r in records
        if isinstance(r.data.get("id"), str)
    }

    for record in records:
        for field, expected_prefix in RELATION_FIELDS.items():
            ...
            for target in values:
                ...
                if not target.startswith(expected_prefix):
                    add(
                        ...
                        "ERROR",
                        record.path,
                        f"relationship '{field}' contains '{target}', "
                ...
                if target not in existing_ids:
                    add(
                        ...
                        "ERROR",
                        record.path,
                        f"relationship '{field}' references missing object "
                        f"'{target}'",
                    )
        ...
        try:
            ...
            if "journal" in record.data:
                add(
                    ...
                    "ERROR",
                    literature_template,
                    "template uses reserved 'journal'; use 'publication'",
                )
            if "author" in record.data:
                add(
                    ...
                    "ERROR",
                    literature_template,
                    "template uses Quarto 'author'; use 'authors'",
                )
            ...
            add(
                ...
                "ERROR",
                literature_template,
                f"cannot parse template: {exc}",
            )


def run_validation(strict: bool = False) -> int:
    findings: list[Finding] = []
    paths = discover_object_paths()
    records: list[Record] = []
    ...
    for path in paths:
        ...
        if path.parts[-3:-2] == ("logs",):
            # Kept for unusual layouts; normal logs are detected below too.
            validate_log(record, findings)
        elif "logs" in path.relative_to(ROOT).parts:
            validate_log(record, findings)
        else:
